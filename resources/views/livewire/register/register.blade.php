<div class="relative min-h-screen w-full flex items-center justify-center p-4 overflow-hidden">
    ...
    <div class="fixed inset-0 z-0">
        <img src="{{ asset('assets/img-tela-register.png') }}" class="w-full h-full object-cover" alt="Background">
    ...
    <div class="relative z-10 w-full max-w-md">
        <div class="backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl p-8 shadow-2xl overflow-hidden relative">
            ...
            <div class="text-center mb-8 relative">
                <h1 class="text-4xl font-black text-white tracking-tight mb-2">Cadastre-se</h1>
                <p class="text-white/60 font-medium tracking-wide">Faça o seu cadastro para acessar</p>
            </div>

            <form wire:submit.prevent="authenticate" class="space-y-6 relative">
                <div>
                    <label for="name" class="block text-sm font-semibold text-white/80 mb-2 ml-1 uppercase tracking-widest text-[10px]">Nome</label>
                    <div class="relative group">
                        <input wire:model="name" type="name" id="name" required
                    ...
                    @error('name') <span class="text-rose-400 text-xs mt-2 ml-1 font-medium">{{ $message }}</span> @enderror
                </div>
                <div>
                ...
                <div>
                    <label for="password" class="block text-sm font-semibold text-white/80 mb-2 ml-1 uppercase tracking-widest text-[10px]">Confirmar senha</label>
                    <div class="relative group">
                        <input wire:model="password" type="password" id="password" required
                               class="w-full bg-white/5 border border-white/10 text-white rounded-2xl px-5 py-4 outline-none focus:border-blue-400 focus:bg-white/10 transition-all duration-300 placeholder:text-white/20"
                               placeholder="********">
        ...
        <div class="mt-8 text-center text-white/30 text-[10px] uppercase tracking-[0.2em] font-medium">
            &copy; {{ date('Y') }} Raul Ito &bull; Todos os direitos reservados
        </div>
    </div>
</div>